    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(isTyrant 
                ? '👑 Ruta del Tirano: Gobiernas con miedo y opresión' 
                : '⚖️ Ruta del Rey: Gobiernas con comercio y paz')
            .addFields(
                { name: '👥 Población', value: settlement.population.toString(), inline: true },
                { name: '💰 Tesoría', value: `${settlement.resources.wood} 🪵 | ${settlement.resources.iron} ⛏️`, inline: true },
                { name: '⚙️ Edificios', value: `Granja: ${settlement.buildings.farm}\n${isTyrant ? 'Barracas' : 'Mercado'}: ${isTyrant ? settlement.buildings.barracks : settlement.buildings.market}` }
            );
