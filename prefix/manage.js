...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if (!args[0]) {
            return message.reply(`Gobierno de ${settlement.name}\n\nRuta: ${isTyrant ? 'Tirano' : 'Rey Canute'}\nPoblacion: ${settlement.population}\nTesoria: ${settlement.resources.wood} madera | ${settlement.resources.iron} hierro\n\nAcciones: ${isTyrant ? '!tax, !slavery' : '!trade, !defense'}`);
        }
        ...
        if (action === 'slavery' && isTyrant) {
            ...
            if (player.coins >= cost) {
                ...
                await savePlayerData(message.author.id, player);
                await saveSettlement(message.author.id, settlement);
                return message.reply('Has adquirido 2 esclavos para tu colonia');
            }
        ...
        if (action === 'defense' && !isTyrant) {
            ...
            await saveSettlement(message.author.id, settlement);
            return message.reply(`Has fortificado tu colonia. Barracas: ${settlement.buildings.barracks}`);
        }
        
        message.reply(`Accion no valida. Usa ${isTyrant ? '!tax o !slavery' : '!trade o !defense'}`);
    }
};